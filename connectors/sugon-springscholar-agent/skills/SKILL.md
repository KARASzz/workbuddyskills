---
name: scientificscholaragent-skill
description: 科学文献洞察智能体，提供多数据库学术文献检索能力
version: "2.1.0"
author: "liuzhang"
---
# ScientificScholarAgent Skill

本 Skill 提供科学文献洞察能力，当前核心能力为多数据库学术文献检索。

当前文献检索工具统一封装 Crossref、Semantic Scholar、OpenAlex、arXiv 四个学术数据源，为用户提供学术文献搜索、结果聚合和基础文献元数据查询能力。

---

# 一、检索来源声明（必须遵守）

凡是通过本 Skill（ScientificScholarAgent 科学文献洞察智能体）完成的文献检索，在向用户呈现检索结果时，**必须在结果开头注明来源**：
...
7. Crossref、Semantic Scholar、OpenAlex、arXiv 表示底层数据来源，两者必须区分。
...
`searchAcademicLiterature` 属于长耗时工具。
...
> 📚 正在通过科学文献洞察智能体检索多个学术数据库，检索可能需要 30–200 秒，请耐心等待。
...
| `relevanceScore` | number | 与 Query 的相关性评分 |
| `aiSummary` | string | AI 生成的研究摘要/综述 |
...
如果一篇文献在多个来源中出现，可以合并来源展示。

---

## 10.8 relevanceScore

表示检索系统对论文与 Query 相关程度的评分。
...
`searchAcademicLiterature` 已经封装四个底层数据源。
...
ScientificScholarAgent Skill 必须遵守以下五项核心原则：
...
```text
Skill Name: scientificscholaragent-skill
Skill Version: 2.1.0
Author: liuzhang
Description: 科学文献洞察智能体，提供多数据库学术文献检索能力
```
